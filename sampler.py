...
def line_bo_simplex_sampler(
    ...
    """
    Main Line-BO sampler, now enforcing a simplex constraint (fractions sum to 1)
    and returning both the best and second-best lines.

    The second-best line is written to the *cache* fields so downstream DiSCO
    can consume it without blocking the main queue.
    """

    def _to_simplex(arr: np.ndarray) -> np.ndarray:
        """Project each vector to the unit simplex."""
    ...
    # guarantee that candidate endpoints respect the simplex
    K_cand = calc_K(
        N,
        M,
        constrain_sum_x=True,  # keeps A, B on the simplex
        plotting=plotting,
    ...
    (
        ...
        emax=emax,
        emin=emin,
        emax_global=emax_global,
        emin_global=emin_global,
        M=M,
        acq_max=acq_max,
        selection_method=selection_method,
        plotting=plotting,
    ...
    # 4) droplets along both lines
    idxs = [0]  # BO-suggested index in X_ask
    x = compute_x_coords_along_lines(
        idxs, N, n_droplets, tA_sel, tB_sel, X_ask, K_sel
    )
    x_cache = compute_x_coords_along_lines(
    ...
    # 5) enforce simplex (each point sums to 1)
    A_sel, B_sel = _to_simplex(A_sel), _to_simplex(B_sel)
    A_cache, B_cache = _to_simplex(A_cache), _to_simplex(B_cache)
    x, x_cache = _to_simplex(x), _to_simplex(x_cache)
    ...
    # 7) hand off to experiment / DiSCO
    communication.write_compositions(
        start=np.round(A_sel, 3),
        end=np.round(B_sel, 3),
        array=np.round(x, 3),
