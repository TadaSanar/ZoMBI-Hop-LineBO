...
def line_bo_sampler(X_ask, x_acquisitions, acquisitions, model, 
    ...
    # 5) logging
    print('best start: ', normalize_last_axis(A_sel))
    print('best end:   ', normalize_last_axis(B_sel))
    # print('best drops: ', x)
    print('cache start:', normalize_last_axis(A_cache))
    print('cache end:  ', normalize_last_axis(B_cache))
    # print('cache drops:', x_cache)

    # 6) hand off to experiment
    communication.write_compositions(
        start=normalize_last_axis(np.round(A_sel,3)),
        end=normalize_last_axis(np.round(B_sel,3)),
        array=normalize_last_axis(np.round(x,3)),
        start_cache=normalize_last_axis(np.round(A_cache,3)),
        end_cache=normalize_last_axis(np.round(B_cache,3)),
        array_cache=normalize_last_axis(np.round(x_cache,3)),
        timestamp=time.time()
    )
