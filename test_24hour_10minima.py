...
def run_variable_trial(num_minima=None, dimensions=None, time_limit_hours=None, resume_uuid=None):
    ...
    if resume_uuid is None:
        ...
        zombihop = ZoMBIHop(
            ...
            penalization_threshold=1.16e-3,
            improvement_threshold_noise_mult=2.5,
            input_noise_threshold_mult=3.2,
        ...
        with open(trial_dir / 'trial_metadata.json', 'r') as f:
    ...
    try:
        ...
        print(f"Resume with: run_24hour_trial(resume_uuid='{zombihop.run_uuid}')")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        # Check if first argument is a UUID (for resuming)
        if len(sys.argv[1]) > 10 and '-' in sys.argv[1]:
            # Looks like a UUID
            resume_uuid = sys.argv[1]
            print(f"Resuming trial with UUID: {resume_uuid}\n")
            run_variable_trial(resume_uuid=resume_uuid)
        else:
            # Parse new trial parameters
            try:
                num_minima = int(sys.argv[1]) if len(sys.argv) > 1 else DEFAULT_NUM_MINIMA
                ...
                print("Usage: python test_24hour_variable_minima.py [num_minima] [dimensions] [time_hours]")
                print("   or: python test_24hour_variable_minima.py [resume_uuid]")
                sys.exit(1)
    else:
