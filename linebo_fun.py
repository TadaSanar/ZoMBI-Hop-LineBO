...
def calc_K(dimensionality, M, constrain_sum_x=False, plotting=False, max_candidates=5000):
    """
    ...
    if K_total <= max_candidates:
        ...
        sph = np.zeros((km.shape[0], N))
        sph[:,0] = 1.0
        if N > 1:
            sph[:,1:-1] = km[:,:-1] * np.pi / M
            sph[:,-1]   = km[:,-1]   * 2*np.pi / M
        cart_coords = n_sphere.convert_rectangular(sph)
    else:
        # — random sampling on sphere instead —
        K = max_candidates
        rand = np.random.normal(size=(K, N))
        cart_coords = rand / np.linalg.norm(rand, axis=1, keepdims=True)

    # optionally plot as before
    if plotting and cart_coords.shape[1] >= 2:
        import matplotlib.pyplot as plt
        plt.scatter(cart_coords[:,0], cart_coords[:,1], s=1)
        plt.title(f"{cart_coords.shape[0]} directions (dim={N})")
        plt.show()

    # lift back to the simplex‐bound if needed
    if constrain_sum_x:
...
def extract_inlet_outlet_points(p, K_cand, emax, emin, M,
    ...
    # broadcast p to (K,D)
    p_rep = np.repeat(p, K, axis=0)    # (K, D)
    Kmat  = K_cand                     # (K, D)
    ...
    tA = np.nanmax(np.where(t_stack <= 0, t_stack, np.nan), axis=1, keepdims=True)
    tB = np.nanmin(np.where(t_stack >= 0, t_stack, np.nan), axis=1, keepdims=True)
...
def integrate_all_K_over_acqf(p, K_cand, t_starts, t_stops, n_points,
    ...
    t_steps  = t_starts.reshape(K,1) * (1 - t_lin) + t_stops.reshape(K,1) * t_lin
    # shape (K, n_points)
